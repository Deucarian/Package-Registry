...
- [ARCHITECTURE.md](ARCHITECTURE.md) is the single canonical architecture
  ...
  local copies.
- [AOT_SAFETY.md](AOT_SAFETY.md) defines the runtime-reflection, AOT evidence,
  generated linker, and production stripping contract. Deucarian-owned player
  code is generated, explicitly composed, exactly declared, or rejected.
...
- `com.deucarian.build-pipeline` owns final-player AOT inspection, evidence verification, exact generated linker input, build manifests, and headless build entry points. Domain packages still own their own generators.
- `generated-json-serialization` is a proposed capability for `com.deucarian.serialization`. It is not installable and must not appear in package dependencies or `packages.json` until its repository and release channels exist.
